...
export const ingestLocalPdf = async ({ filePath, standardNumber, standardTitle, sourceUrl, documentVersion }) => {
  ...
  const standard = await prisma.standard.upsert({
    where: { number: required(standardNumber, 'standardNumber') },
    update: { title: required(standardTitle, 'standardTitle') },
    create: { number: standardNumber.trim(), title: standardTitle.trim() },
  });
  const document = await prisma.standardDocument.create({
    data: { standardId: standard.id, fileName, fileType: 'application/pdf', filePath: resolvedPath, sourceUrl: sourceUrl || null, documentVersion: documentVersion || null, extractionStatus: 'PENDING' },
  });

  try {
    await prisma.standardDocument.update({ where: { id: document.id }, data: { extractionStatus: 'PROCESSING' } });
    const extractedPages = await extractPdfPages(resolvedPath);
    const cleanedPages = extractedPages.map((page) => ({ ...page, text: cleanPageText(page.text) })).filter((page) => page.text);
    ...
    await prisma.$transaction([
      ...
      prisma.standardDocument.update({ where: { id: document.id }, data: { pageCount: extractedPages.length, extractionStatus: 'COMPLETED' } }),
    ]);
    return { documentId: document.id, pages: extractedPages.length, chunks: chunks.length, status: 'COMPLETED' };
  } catch (error) {
    await prisma.standardDocument.update({ where: { id: document.id }, data: { extractionStatus: 'FAILED' } });
    throw error;
  }
};
